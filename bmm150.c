...
static bmm150_err_t bmm150_bus_initialize(bmm150_t const *bmm150) {
  return bmm150->interface.bus_initialize
             ? bmm150->interface.bus_initialize(bmm150->interface.bus_user)
             : BMM150_ERR_NULL;
}

static bmm150_err_t bmm150_bus_deinitialize(bmm150_t const *bmm150) {
  return bmm150->interface.bus_deinitialize
             ? bmm150->interface.bus_deinitialize(bmm150->interface.bus_user)
             : BMM150_ERR_NULL;
}

static bmm150_err_t bmm150_bus_write_data(bmm150_t const *bmm150,
                                          uint8_t address, uint8_t const *data,
                                          size_t data_size) {
  return bmm150->interface.bus_write_data
             ? bmm150->interface.bus_write_data(bmm150->interface.bus_user,
                                                address, data, data_size)
             : BMM150_ERR_NULL;
}

static bmm150_err_t bmm150_bus_read_data(bmm150_t const *bmm150,
                                         uint8_t address, uint8_t *data,
                                         size_t data_size) {
  return bmm150->interface.bus_read_data
             ? bmm150->interface.bus_read_data(bmm150->interface.bus_user,
                                               address, data, data_size)
             : BMM150_ERR_NULL;
}

bmm150_err_t bmm150_initialize(bmm150_t *bmm150, bmm150_config_t const *config,
  ...
  return bmm150_bus_initialize(bmm150);
}

bmm150_err_t bmm150_deinitialize(bmm150_t *bmm150) {
  assert(bmm150);

  bmm150_err_t err = bmm150_bus_deinitialize(bmm150);
...
bmm150_err_t bmm150_get_mag_data_x_scaled(bmm150_t const *bmm150,
                                          float32_t *scaled) {
...
bmm150_err_t bmm150_get_mag_data_y_scaled(bmm150_t const *bmm150,
                                          float32_t *scaled) {
  assert(bmm150 && scaled);
...
bmm150_err_t bmm150_get_mag_data_z_scaled(bmm150_t const *bmm150,
                                          float32_t *scaled) {
  assert(bmm150 && scaled);
...
bmm150_err_t bmm150_get_mag_data_xyz_scaled(bmm150_t const *bmm150,
...
bmm150_err_t bmm150_get_mag_data_xyz_raw(bmm150_t const *bmm150,
                                         vec3_int16_t *raw) {
  assert(bmm150 && raw);
...
bmm150_err_t bmm150_get_trim_data(bmm150_t const *bmm150,
                                  bmm150_trim_data_t *data) {
  assert(bmm150 && data);
  ...
  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DIG_X1,
                                          trim_x1y1, sizeof(trim_x1y1));
  err |= bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DIG_Z4_LSB, trim_xyz,
                              sizeof(trim_xyz));
  err |= bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DIG_Z2_LSB,
  ...
  data->dig_xyz1 =
      (uint16_t)(((uint16_t)(trim_xy1xy2[5] & 0x7F) << 8) | trim_xy1xy2[4]);

  return err;
}

bmm150_err_t bmm150_get_chip_id_reg(bmm150_t const *bmm150,
                                    bmm150_chip_id_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_CHIP_ID,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_get_data_x_reg(bmm150_t const *bmm150,
                                   bmm150_data_x_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data[2] = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DATAX_LSB,
                                          data, sizeof(data));
...
bmm150_err_t bmm150_get_data_y_reg(bmm150_t const *bmm150,
                                   bmm150_data_y_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data[2] = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DATAY_LSB,
                                          data, sizeof(data));
...
bmm150_err_t bmm150_get_data_z_reg(bmm150_t const *bmm150,
                                   bmm150_data_z_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data[2] = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_DATAZ_LSB,
                                          data, sizeof(data));
...
bmm150_err_t bmm150_get_data_xyz_reg(bmm150_t const *bmm150,
                                     bmm150_data_xyz_reg_t *reg) {
  assert(bmm150 && reg);
...
bmm150_err_t bmm150_get_rhall_reg(bmm150_t const *bmm150,
                                  bmm150_rhall_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data[2] = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_RHALL_LSB,
                                          data, sizeof(data));

  reg->rhall =
      (uint16_t)(((data[0] >> 2U) & 0x3FU) | ((data[1] & 0xFFU) << 6U));
  reg->data_ready_status = data[1] & 0x01U;

  return err;
}

bmm150_err_t bmm150_get_int_stat_reg(bmm150_t const *bmm150,
                                     bmm150_int_stat_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_INT_STAT,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_get_pwr_rst_reg(bmm150_t const *bmm150,
                                    bmm150_pwr_rst_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_PWR_RST,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_set_pwr_rst_reg(bmm150_t const *bmm150,
                                    bmm150_pwr_rst_reg_t const *reg) {
  assert(bmm150 && reg);
  ...
  err |= bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_PWR_RST, &data,
                               sizeof(data));

  return err;
}

bmm150_err_t bmm150_get_odr_opmode_reg(bmm150_t const *bmm150,
                                       bmm150_opmode_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_OPMODE,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_set_odr_opmode_reg(bmm150_t const *bmm150,
                                       bmm150_opmode_reg_t const *reg) {
  assert(bmm150 && reg);
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_OPMODE, &data,
                               sizeof(data));
}

bmm150_err_t bmm150_get_int_ctrl1_reg(bmm150_t const *bmm150,
                                      bmm150_int_ctrl1_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_INT_CTRL1,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_set_int_ctrl1_reg(bmm150_t const *bmm150,
                                      bmm150_int_ctrl1_reg_t const *reg) {
  assert(bmm150 && reg);
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_INT_CTRL1, &data,
                               sizeof(data));
}

bmm150_err_t bmm150_get_int_ctrl2_reg(bmm150_t const *bmm150,
                                      bmm150_int_ctrl2_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_INT_CTRL2,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_set_int_ctrl2_reg(bmm150_t const *bmm150,
                                      bmm150_int_ctrl2_reg_t const *reg) {
  assert(bmm150 && reg);
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_INT_CTRL2, &data,
                               sizeof(data));
}
...
bmm150_err_t bmm150_get_int_low_thresh_reg(bmm150_t const *bmm150,
  ...
  bmm150_err_t err = bmm150_bus_read_data(
      bmm150, BMM150_REG_ADDRESS_INT_LOW_THRESH, &data, sizeof(data));
...
bmm150_err_t
bmm150_set_int_low_thresh_reg(bmm150_t const *bmm150,
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_INT_LOW_THRESH, &data,
                               sizeof(data));
}
...
bmm150_err_t bmm150_get_int_high_thresh_reg(bmm150_t const *bmm150,
  ...
  bmm150_err_t err = bmm150_bus_read_data(
      bmm150, BMM150_REG_ADDRESS_INT_HIGH_THRESH, &data, sizeof(data));
...
bmm150_err_t
bmm150_set_int_high_thresh_reg(bmm150_t const *bmm150,
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_INT_HIGH_THRESH,
                               &data, sizeof(data));
}

bmm150_err_t bmm150_get_axis_rep_xy_reg(bmm150_t const *bmm150,
                                        bmm150_axis_rep_xy_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(
      bmm150, BMM150_REG_ADDRESS_AXIS_REP_XY, &data, sizeof(data));
...
bmm150_err_t bmm150_set_axis_rep_xy_reg(bmm150_t const *bmm150,
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_AXIS_REP_XY, &data,
                               sizeof(data));
}

bmm150_err_t bmm150_get_axis_rep_z_reg(bmm150_t const *bmm150,
                                       bmm150_axis_rep_z_reg_t *reg) {
  assert(bmm150 && reg);

  uint8_t data = {};

  bmm150_err_t err = bmm150_bus_read_data(bmm150, BMM150_REG_ADDRESS_AXIS_REP_Z,
                                          &data, sizeof(data));
...
bmm150_err_t bmm150_set_axis_rep_z_reg(bmm150_t const *bmm150,
                                       bmm150_axis_rep_z_reg_t const *reg) {
  assert(bmm150 && reg);
  ...
  return bmm150_bus_write_data(bmm150, BMM150_REG_ADDRESS_AXIS_REP_Z, &data,
                               sizeof(data));
}
...
bmm150_err_t bmm150_send_softreset_cmd(bmm150_t const *bmm150) {
  ...
  return bmm150_bus_write_data(bmm150, data, NULL, 0UL);
}
