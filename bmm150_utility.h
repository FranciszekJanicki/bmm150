                                        ...
                                        int16_t raw)
{
    ...
    if ((raw != BMM150_OVERFLOW_XY_FLIP) && (rhall != 0) &&
        (data->dig_xyz1 != 0)) {
        process_comp[0] = (((float32_t)data->dig_xyz1) * 16384.0f / rhall);
        scaled = process_comp[0] - 16384.0f;
        process_comp[1] =
            ((float32_t)data->dig_xy2) * (scaled * scaled / 268435456.0f);
        process_comp[2] =
            process_comp[1] + scaled * ((float32_t)data->dig_xy1) / 16384.0f;
        process_comp[3] = ((float32_t)data->dig_x2) + 160.0f;
        process_comp[4] = raw * ((process_comp[2] + 256.0f) * process_comp[3]);
        scaled =
            ((process_comp[4] / 8192.0f) + (((float32_t)data->dig_x1) * 8.0f)) *
            BMM150_LSB_SCALE;
    }
...
inline float32_t bmm150_y_raw_to_scaled(bmm150_trim_data_t const* data,
                                        uint16_t rhall,
                                        int16_t raw)
{
    assert(data);
    ...
    if ((raw != BMM150_OVERFLOW_XY_FLIP) && (rhall != 0) &&
        ...
        process_comp[4] = raw * ((process_comp[2] + 256.0f) * process_comp[3]);
        scaled =
            ((process_comp[4] / 8192.0f) + (((float32_t)data->dig_y1) * 8.0f)) *
            BMM150_LSB_SCALE;
    }
...
inline float32_t bmm150_z_raw_to_scaled(bmm150_trim_data_t const* data,
                                        uint16_t rhall,
                                        int16_t raw)
{
    assert(data);
    ...
    if ((raw != BMM150_OVERFLOW_Z_HALL) && (data->dig_z2 != 0) &&
        (data->dig_z1 != 0) && (data->dig_xyz1 != 0) && (rhall != 0)) {
